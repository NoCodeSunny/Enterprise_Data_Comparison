...
class EnterpriseAuditLogger:
    ...
    def _write(self, batch_id: str, payload: Dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            # Note: file stays writable during batch execution (multiple writes possible).
            # Call finalize_batch() after terminal state to make it immutable.


    def finalize_batch(self, batch_id: str) -> None:
        """
        Make the audit log immutable (chmod 444) after a batch reaches a terminal state.

        This is a best-effort operation — called once after SUCCESS, FAILED,
        PARTIAL_SUCCESS, or CANCELLED. Safe to call multiple times.
        """
        from datetime import datetime, timezone
        # Find log file (use today's date since terminal state is logged today)
        now = datetime.now(timezone.utc)
        log_path = self._log_path(batch_id, now)
        if log_path.exists():
            try:
                ...
                pass  # Best-effort: read-only FS, root, or Windows
