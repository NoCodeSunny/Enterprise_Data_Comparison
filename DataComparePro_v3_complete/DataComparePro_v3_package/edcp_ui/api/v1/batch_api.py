...
@v1.route("/batch/validate", methods=["POST"])
def validate_batch_only():
    """POST /api/v1/batch/validate — Validate comparisons WITHOUT creating a batch.
    
    This is a pure dry-run validation endpoint. No batch is created, no state is
    changed. Returns the same error format as POST /api/v1/batch.
    """
    t0 = time.time()
    body = request.get_json(force=True, silent=True) or {}
    comparisons = body.get("comparisons", [])
...
def get_job(job_id: str):
    ...
    for batch in batches:
        if batch_id_hint and batch.batch_id != batch_id_hint:
            continue
        for job in batch.jobs:
            if job.job_id == job_id:
                d = job.to_dict()
                d["batch_id"] = batch.batch_id
                d["logs_url"] = f"/api/v1/job/{job_id}/logs?batch_id={batch.batch_id}"
                return _ok(d, t0=t0)
...
def get_job_logs(job_id: str):
    ...
    Accepts optional ?batch_id=XXX for fast lookup when batch is known.
    """
    t0     = time.time()
...
@v1.route("/batch/<batch_id>/report/<report_type>", methods=["GET"])
def download_batch_report(batch_id: str, report_type: str):
    """GET /api/v1/batch/{id}/report/{type} — Download batch-level report.
    
    Aggregates reports from all jobs in the batch into a zip download,
    or returns the first available report of the requested type.
    """
    import zipfile, io
    ...
    # Collect all matching reports across jobs
    report_files = []
    for job in batch.jobs:
        rp = _REPORT_ROOT / batch_id / job.job_id
        ext_map = {"excel": ".xlsx", "html": ".html", "json": ".json"}
        ext = ext_map.get(report_type.lower())
        if ext:
            for f in rp.rglob(f"*{ext}"):
                report_files.append((job.job_id, f))

    if not report_files:
        return _err("REPORT_NOT_FOUND",
                    f"No {report_type} reports found for batch {batch_id}.",
                    suggestion="Ensure the batch completed successfully before downloading.",
                    status=404)

    if len(report_files) == 1:
        jid, fp = report_files[0]
        return _sf(str(fp), as_attachment=True,
                   download_name=f"{batch_id}_{jid}_{fp.name}")

    # Multiple files — zip them
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        for jid, fp in report_files:
            zf.write(str(fp), f"{jid}/{fp.name}")
    buf.seek(0)
    return _sf(buf, as_attachment=True,
               download_name=f"{batch_id}_{report_type}_reports.zip",
