...
class BatchManager:
    ...
    def retry_jobs(self, batch_id: str, job_ids: List[str], max_attempts: int = 3) -> BatchRecord:
        """Create a retry batch for specific failed jobs."""
        batch = self._get_batch(batch_id)
        to_retry = [j for j in batch.jobs if j.job_id in job_ids and j.status == JobStatus.FAILED]
        if not to_retry:
            raise ValueError("No failed jobs found matching the given IDs.")
        # Reset and requeue
        for job in to_retry:
            job.status      = JobStatus.QUEUED
            job.retry_count = 0
    ...
    def _execute_batch(self, batch: BatchRecord) -> None:
        ...
        try:
            ...
            if batch.status not in (BatchStatus.CANCELLED, BatchStatus.FAILED):
